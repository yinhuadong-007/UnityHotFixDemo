...
public class BuildAB : Editor
{
    ...
    {
        UnityEngine.Debug.Log("GetCurrentDirectory = " + Directory.GetCurrentDirectory());
        string toPath = Path.Combine(Directory.GetCurrentDirectory(), BuildConfig.BUILD_TO_NAME, BuildConfig.VERSION_FILE_NAME);
        UnityEngine.Debug.Log("toPath = " + toPath);
    ...
    private static void CreateIndexFile()
    {
        ...
        string toPath = Path.Combine(GetOutputPath(), BuildConfig.ASSET_LIST_FILE_NAME);
    ...
    private static void StartBuild()
    {
        ...
        //构建编辑器中指定的所有 AssetBundle, 输出到 outputPath目录下， 
    ...
    {
        string path = Path.Combine(Directory.GetCurrentDirectory(), BuildConfig.BUILD_TO_NAME, Application.version);
    ...
    {
        string outputPath = Path.Combine(Directory.GetCurrentDirectory(), BuildConfig.BUILD_TO_NAME);
        Process.Start(outputPath);
    }
...
//@ https://github.com/1479136868/HotFixDemo